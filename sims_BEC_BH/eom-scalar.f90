...
contains
  ...
  subroutine derivs(yc, yp)
    ...
    if (interaction == 0.) then !! free field: m^2 phi(x) term in eom
      ...
      ! this is the BEC potential; diagonal
      ! here I multiply each entry by the conformal factor
      do x = 1, nLat
          if (x < nLat/2) then
               yp(nLat+x) = - Omega1(x - nLat/2) * V0 * ( sin(yc(x)) + 0.5_dl*delta**2*sin(2._dl*yc(x)) )
          else
               yp(nLat+x) = - Omega2(x - nLat/2) * V0 * ( sin(yc(x)) + 0.5_dl*delta**2*sin(2._dl*yc(x)) )
          endif
      enddo
   endif
