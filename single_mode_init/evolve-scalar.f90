...
!    fld(:,1) = twopi * 0.5_dl ! BEC potential
  ...
  subroutine initialize_fluctuations(fld, sim)
    ...
    spec1(kcsim:kmsim) = norm / w2(kcsim:kmsim)**0.25
    spec2(kcsim:kmsim) = spec1(kcsim:kmsim) * w2(kcsim:kmsim)**0.5
  ...
  subroutine output_fields(fld, dt, dtout, sim)
    ...
    inquire(file='/gpfs/dpirvu/dilatonBH/single_mode_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/dilatonBH/single_mode_sim'//trim(str(sim))//'_fields.dat')
