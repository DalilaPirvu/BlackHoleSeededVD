...
contains
  ...
  subroutine initialise_fields(fld)
    ...
    fld(:,1) = 0._dl ! free field starts off at zero
    fld(:,2) = 0._dl
...
!    enddo
    ...
    spec1(kcmin:kcmax) = norm / w2(kcmin:kcmax)**0.25
  ...
  subroutine output_fields(fld, dt, dtout, sim)
    ...
    inquire(file='/gpfs/dpirvu/dilatonBH/test_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/dilatonBH/test_sim'//trim(str(sim))//'_fields.dat')
